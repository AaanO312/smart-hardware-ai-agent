...
- **LLM & 模型**：通义千问（DashScope）、DeepSeek-V4；API 调用、参数调优、流式输出
- **向量库 & 检索**：Chroma、DashScope Embedding、PDF/TXT 文档解析（PyPDF）、文本切分、Top-K 语义检索
- **工程 & 工具**：Python、Streamlit、FastAPI、Git、异常处理与全链路排查
...
- 基于 RAG 搭建 Chroma 向量库，PDF/TXT 解析 → 文本切分 → DashScope Embedding 向量化 → Top-K 检索，导入 3 个扫地机器人知识库文件
...
- 4 Agent 协作：天气查询（Open-Meteo 实时 API）→ 行程规划 ∥ 预算估算（并行 Fan-out）→ 协调 Agent 智能审核（6 项标准：天气冲突/预算超支/行程密度/时间逻辑/偏好匹配/完整度）
- 协调 Agent 审核不通过时自动驳回、生成修订指令、行程/预算 Agent 按指令重做、再次提交，最多 3 轮循环直到自洽
- 表单收集需求 → Agent 生成方案 → 对话式微调，用户反馈直接注入修订循环
